...
class MapDetailFragment(placeId: Int) : BaseFragment<FragmentMapDetailBinding>(FragmentMapDetailBinding::bind, R.layout.fragment_map_detail) {

	private val place = placeId
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		super.onViewCreated(view, savedInstanceState)
		getMapDetail(place)
		binding.mapDetailReviewRv.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false)
		binding.mapDetailReviewRv.setHasFixedSize(true)
	}

	fun getMapDetail(placeId : Int){
		val mapDetailRetrofitInterface = ApplicationClass.sRetrofit.create(MapDetailRetrofitInterface::class.java)
		mapDetailRetrofitInterface.getMapDetail(placeId).enqueue(object : Callback<MapDetail>{
			...
			override fun onResponse(call: Call<MapDetail>, response: Response<MapDetail>) {
				...
				binding.mapDetailReviewCntTv.text = binding.mapDetailReviewCntTv.text.toString() + result.result.reviewCount.toString()
				binding.mapDetailCallTv.text = result.result.phoneNumber
				binding.mapDetailAddTv.text = result.result.address
				binding.mapDetailReviewTv.text =  binding.mapDetailReviewTv.text.toString() + result.result.reviewCount.toString()
			}
